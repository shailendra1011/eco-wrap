...
var table_data=[];
    ...
    `;
        ...
        $.ajax({
            url: link ? link : "{{ route('pharmacy.orders') }}",
            type: 'get',
            data: {
                _token: "{{ csrf_token() }}",
                from_date: from_date,
                ...
                if (res.data.data.length) {
                    ...
                    $.each(res.orders, function (key, value) {
                        ...
                        <tr>
                            ...
                            if(value.driver_status){
                            row+=` <td><a  href="javascript:void(0)"   onclick="driverDetails(${value.order_id})" data-toggle="modal" data-target="#myModalDetails">${value.driver_name}</a></td>`;
                            }else{
                                row+=`<td>${value.driver_name}</td>`;
                            }
                           row+=` 
                            <td>
   ...
   swal({
       ...
       $.ajax({
        ...
        function driverDetails(id)
        {
            ...
            row+=`
            <div class="modal inmodal" id="myModalDetails" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-md">
                    <div class="modal-content animated bounceInRight">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                            
                            <h4 class="modal-title">{{Session::get('locale')=='en'?'Driver details':'Detalles del conductor'}} </h4>
                            
                        </div>
                        <div class="modal-body">                            
                            <div class='row'>
                                <div class="col-8">
                                    <p><strong>Driver name :</strong> ${details.driver_order.driver_details.driver_name??''}</p>
                                </div>
                            </div>                                                        
                        </div>                        
                    </div>
