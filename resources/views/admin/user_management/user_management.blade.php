...
<script>
    ...
    function get_data(link = null) {
        ...
        $.ajax({
            url: link != null ? link : "{{route('user/data')}}",
            type: "post",
            data: {
                _token: "{{ csrf_token() }}",
                search: search,
                ...
                if (res.data.data.length) {
                    ...
                    $.each(res.data.data, function(key, value) {
                        ...
                        row += `
							...
							<td> ${res.data.from++} </td>`
                        if (value.user_image) {
                            row += `<td><img src="${value.user_image}" style="height:40px;width:40px; margin-right:2px; border-radius:50%"></td>`
                        } else {
                            row += `<td><img src= "{{asset('admin/img/logoVector.svg ')}}" style="height:40px;width:40px; margin-right:2px; border-radius:50%"></td>`
                        }
                        row += `<td> ${value.name? value.name : '-'}</td>
